...
######### Baja páginas, grafica y envía alarmas
def procesa_juegos():
    plt.ioff()
    conn = conecta_db()
    cursor = conn.cursor()
    cursor.execute('SELECT DISTINCT BGG_id, nombre FROM juegos WHERE BGG_id = 39856 OR BGG_id = 158899 ORDER BY nombre') ###########
    ...
    for jb in juegos_BGG: # Cada juego diferente
        ...
        for j in juegos_id: # Cada repetición del mismo juego
            ...
            if any(precio_hi): # Si hay algún dato válido, grafica
...
######### Exporta los resultados a un csv
def exporta():
    conn = conecta_db()
    cursor = conn.cursor()
...
def procesea_juegos_prioridad_1():
    procesa_juegos()
...
bot_token = "1849534953:AAEkC1t9vBVoCOm-JxdSouv-NEEPj2fAk_I"
updater = Updater(bot_token)
schedule.every(2).minutes.do(run_threaded, procesea_juegos_prioridad_1)
schedule.every(3).minutes.do(run_threaded, procesea_juegos_prioridad_1)

while True:
    schedule.run_pending()
    time.sleep(1)

# procesea_juegos_prioridad_1()